...
# ── MUDELI SEADED ─────────────────────────────────────────────────────────────
# Groq lõpetas llama-3.3-70b-versatile toe (juuni 2026).
# Mudelit saab vahetada Streamlit Cloudi Secrets alt: GROQ_MODEL = "..."
# ilma koodi muutmata.

VAIKEMUDEL = "openai/gpt-oss-120b"
VARUMUDEL = "qwen/qwen3.6-27b"


def saa_mudel() -> str:
    """Loeb mudeli nime Secrets'ist, muidu kasutab vaikemudelit."""
    try:
        return st.secrets.get("GROQ_MODEL", VAIKEMUDEL)
    except Exception:
        return VAIKEMUDEL
...
@st.cache_resource
def saa_klient() -> Groq:
    """Üks Groq klient kogu rakenduse peale."""
    return Groq()


def groq_vastus(messages, max_tokens=1000, temperature=0.7, stream=False):
    """Küsib vastuse Groq API-lt. Kui põhimudel pole saadaval, proovib varumudelit."""
    client = saa_klient()
    kasutatav = st.session_state.get("valitud_mudel", MUDEL)
    try:
        ...
        # Nt kui mudel on aegunud (404) või kontole pole kättesaadav
        return client.chat.completions.create(
            model=VARUMUDEL,
            messages=messages,
            max_tokens=max_tokens,
...
.stMarkdown p { font-size: 16px !important; color: #1A2E1C !important; }
...
# Ühine eesti keele reeglistik — lisatakse KÕIGILE promptidele.
# Vajalik, sest mudelid kipuvad eesti keelde tooma saksa- ja ingliskeelseid vorme.

KEELEREEGLID = """

EESTI KEELE NÕUDED — KOHUSTUSLIK, KEHTIB KOGU VASTUSELE:
- Kirjuta AINULT eesti keeles. Mitte ükski sõna ei tohi olla saksa, inglise ega vene keeles.
- Kuunimed on eesti keeles: jaanuar, veebruar, märts, aprill, mai, juuni, juuli,
  august, september, oktoober, november, detsember.
  ❌ VALE: februar, februaar, märz, august'is, September
  ✅ ÕIGE: veebruar, veebruaris, veebruari kuu, märts, märtsis
- Kuunimed ja nädalapäevad kirjutatakse VÄIKESE algustähega: veebruar, esmaspäev, reede.
- Nädalapäevad: esmaspäev, teisipäev, kolmapäev, neljapäev, reede, laupäev, pühapäev.
- Kuupäev kirjutatakse nii: 12. veebruar 2026 või 12.02.2026.
- Kasuta õigeid täpitähti: ä, ö, ü, õ, š, ž. Ära kirjuta 'a' asemel 'ä' ega vastupidi.
- Kasuta grammatiliselt õiget käänet — näiteks 'aidata sind', mitte 'aidata sul'.
- Kui sa ei ole mõne sõna õiges vormis kindel, kasuta lihtsamat ja tavalisemat sõna.
  Parem lihtne õige lause kui keeruline vigane lause.
- Enne vastuse lõpetamist loe oma tekst mõttes üle ja paranda õigekirjavead."""
...
- Räägi eesti keeles
- Kasuta alati grammatiliselt õiget eesti keelt — näiteks 'aidata sind', mitte 'aidata sul'""" + KEELEREEGLID
...
Kõigepealt lihtne tekst, siis lühike selgitus mis muudeti.""" + KEELEREEGLID
...
Väljasta materjal kohe — ilma sissejuhatuse või selgituseta.""" + KEELEREEGLID
...
with st.sidebar:
    ...
    <hr style='border:none; border-top:1px solid #2A5035; margin:0.875rem 0;'>
    <div style='font-size:0.62rem; color:#A0C0A5; text-transform:uppercase;
                letter-spacing:0.1em; margin-bottom:0.25rem; padding:0 0.25rem;'>
        Keelemudel
    </div>
    """, unsafe_allow_html=True)

    MUDELIVALIK = [VAIKEMUDEL, VARUMUDEL, "openai/gpt-oss-20b"]
    if MUDEL not in MUDELIVALIK:
        MUDELIVALIK.insert(0, MUDEL)

    st.selectbox(
        "Keelemudel",
        options=MUDELIVALIK,
        index=MUDELIVALIK.index(MUDEL),
        key="valitud_mudel",
        label_visibility="collapsed",
        help="Mudelid kirjutavad eesti keelt erineva kvaliteediga. "
             "Kui vastustes on keelevigu, proovi teist mudelit.",
    )

    st.markdown("""
    <div style='margin-top:0.5rem; padding:0.625rem 0.75rem; background:#0D1A0F;
                border-radius:8px; border:1px solid #2A5035;'>
        <div style='font-size:0.62rem; color:#6A9A70;'>⚡ Powered by Groq</div>
    </div>
...
# ── TEEMA DETAILLEHT ──────────────────────────────────────────────────────────
    ...
    if teema:
        ...
        with tab_a:
            ...
            # Esita küsimus assistendile nupp
            ...
            if st.button(f"💬 Esita küsimus assistendile {gen} kohta", key="ask_ai_teema"):
                st.session_state.leht = "assistent"
...
Loo täielik, kooliks valmis materjal. Struktuur peab olema selge ja visuaalselt loetav."""
                ...
                with st.spinner("🎓 Loon materjali..."):
                    try:
                        ...
                        st.session_state["mat_viimane_erivajadus"] = erivajadus
                        st.session_state["mat_viimane_teema"] = teema_sisend
                        st.session_state["mat_viimane_klass"] = klass
    ...
    with col_tulemus:
        ...
        if st.session_state.materjal_tulemus:
            # Kasuta neid parameetreid, millega materjal tegelikult loodi
            n_erivajadus = st.session_state.get("mat_viimane_erivajadus", erivajadus)
            n_teema = st.session_state.get("mat_viimane_teema", teema_sisend)
            n_klass = st.session_state.get("mat_viimane_klass", klass)

            is_dyslexia = "üsleksia" in n_erivajadus  # catches Düsleksia
...
{st.session_state.materjal_tulemus}
            ...
            with d1:
                st.download_button(
                    ...
                    file_name=f"oppematerjal_{str(n_teema)[:20].replace(' ','_')}.txt",
                    mime="text/plain",
            ...
            if st.button("💬 Esita küsimus assistendile selle materjali kasutamise kohta", use_container_width=True, key="mat_to_chat"):
                st.session_state.leht = "assistent"
                st.session_state.kiirteema = f"Vaatasin just loodud õppematerjali teemal '{n_teema}' ({n_erivajadus}, {n_klass}). Kas saad anda nõu, kuidas seda tundi kasutada?"
                st.rerun()
        else:
            st.markdown("""
            <div style='background:#fff; border:1px solid #B0CEB3; border-radius:14px;
...
# ── ÜLESANNETE DIFERENTSEERIMINE ─────────────────────────────────────────────
    ...
    if st.button("← Tagasi avalehele", key="back_from_dif"):
        st.session_state.leht = "avaleht"
        st.rerun()
    ...
    for i in range(0, len(DIFERENTSEERI_TEEMAD), 2):
        ...
        for col, j in [(c1, i), (c2, i + 1)]:
            ...
            with col:
                ...
                if st.button(f"💬 Esita küsimus assistendile {gen2} kohta", key=f"dif_chat_{j}", use_container_width=True):
                    st.session_state.leht = "assistent"
...
Lapsena oli tal väga raske ja ta tundis end sügavalt üksikuna.
...
Täna on Nick Vujicic üks maailma kuulsamaid motivatsioonikõnelejaid. Ta on esinenud üle 60 riigis. Ta on kohtunud presidentide ja kuningatega. Ta on abielus, tal on lapsed.
...
Helen õppis lugema, kirjutama, rääkima. Ta astus ülikooli ja sai esimeseks kurtpimedaks inimeseks, kes omandas bakalaureusekraadi. Ta kirjutas 12 raamatut. Ta reisis 39 riiki.
...
Frida ütles: **"Jalgadel on valu, aga hing ei tee mulle haiget ja ma saan maalida."** 🎨""",
        ...
        },
        {
            ...
            "erivajadus": "Kõnearengu hilinemine",
            "ikoon": "⚛️",
            "värv": "#4A8A5C",
            "lugu": """Albert Einstein hakkas rääkima tavalisest hiljem. Väiksena kordas ta lauseid mitu korda, enne kui jätkas.

Kooliõpetajad arvasid, et ta on aeglane. Tema iseseisev mõtlemine ei sobinud tolleaegse kooli rangesse korda.

Esimesel katsel ei võetud teda soovitud ülikooli vastu. Pärast lõpetamist ei leidnud ta pikka aega akadeemilist tööd ja töötas patendiametis.

Just seal, vabadel hetkedel mõeldes, tuli tal füüsika suurim läbimurre.
...
Einstein ütles: **"Kujutlusvõime on olulisem kui teadmised."** ⚛️""",
        ...
        },
        {
            ...
            "ikoon": "🤸",
            "värv": "#8A4A8A",
            "lugu": """2016. aastal lekkisid häkkerid avalikkusele Simone Bilesi terviseandmed, püüdes tema ADHD-ravimeid tema vastu pöörata.

Simone vastas avalikult ja rahulikult: **"Mul on ADHD ja ma olen võtnud selleks ravimeid — selles ei ole midagi häbeneda."**

Simone Biles on üks kõigi aegade parimaid võimlejaid. Kümneid maailmameistritiitleid ja olümpiamedaleid. Ta on teinud elemendid, mis on nimetatud tema järgi — "Biles", "Biles II" — sest teised ei suuda neid järele teha.

Ta on ka üks esimesi tippsportlasi, kes rääkis avalikult vaimse tervise tähtsusest — 2021. aastal Tokyo olümpiamängudel astus ta osadest võistlustest kõrvale, et enda eest hoolt kanda.

Tema julgus — nii võistlusplatsil kui elus — on inspireerinud miljoneid. 🤸""",
        ...
        },
        {
            ...
            "ikoon": "🚀",
            "värv": "#B06040",
            "lugu": """Richard Branson lahkus koolist 16-aastaselt. Lugemine ja arvutamine käisid tal raskelt — tal on düsleksia.

Ta asutas üle 400 ettevõtte. Virgin Records, Virgin Atlantic, Virgin Galactic. Ta on purjetanud ümber maailma ja ületanud Atlandi ookeani õhupalliga.

Branson ütleb: **"Düsleksia andis mulle erioskused. Ma ei suuda meelde jätta pikki tekste, aga ma näen suurt pilti kiiremini kui paljud teised."**

Ta on oma ettevõtete juhtimiseks loonud süsteemid ja meeskonna, mis tema düsleksiat tasakaalustavad. See on tema meelest eelis, mitte puudus. 🚀""",
...
Georgetowni ülikoolis oli ta esimene amputeeritud sportlane, kes võistles NCAA I divisjoni tasemel.

1996. aastal osales ta paralümpiamängudel ja püstitas maailmarekordeid.

Seejärel astus ta ka moemaailma — Alexander McQueen tegi talle puust nikerdatud proteesid, millega ta kõndis moelaval.

Aimee ütleb: **"Sõna 'puudega' eeldab, et midagi on puudu. Mina ei tunne, et mul oleks midagi puudu. Minu proteesid on vahendid, mis annavad mulle rohkem võimalusi, mitte vähem."** 🏃""",
